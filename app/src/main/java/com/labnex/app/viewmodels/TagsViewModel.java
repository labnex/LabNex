...
public class TagsViewModel extends ViewModel {
	...
	private final MutableLiveData<List<TagsItem>> tagList = new MutableLiveData<>(null);
	...
	public LiveData<List<TagsItem>> getTagList() {
		return tagList;
	}
	...
	public void loadTags(Context ctx, long projectId) {
	...
	private void fetch(Context ctx, int page) {
		Call<List<TagsItem>> call =
				RetrofitClient.getApiInterface(ctx).getProjectTags(currentId, resultLimit, page);

		call.enqueue(
				new Callback<>() {
					@Override
					public void onResponse(
							@NonNull Call<List<TagsItem>> c, @NonNull Response<List<TagsItem>> r) {
						ApiResponseHandler.handleFetch(
								r,
								isLoading,
								() -> {
									String totalHeader = r.headers().get("x-total");
									List<TagsItem> body = r.body();
									List<TagsItem> current =
											(page == 1)
													? new ArrayList<>()
													: tagList.getValue() != null
															? new ArrayList<>(tagList.getValue())
															: new ArrayList<>();
									if (body != null) current.addAll(body);
									tagList.setValue(current);
									checkLastPage(
					...
					@Override
					public void onFailure(@NonNull Call<List<TagsItem>> c, @NonNull Throwable t) {
						isLoading.setValue(false);
						isLoadingMore = false;
						if (page == 1) tagList.setValue(new ArrayList<>());
						error.setValue(t.getMessage());
					}
				});
	}

	public void deleteTag(Context ctx, long projectId, String tagName) {
		isActionLoading.setValue(true);
		RetrofitClient.getApiInterface(ctx)
				.deleteProjectTag(projectId, tagName)
				.enqueue(
						new Callback<>() {
							@Override
							public void onResponse(
	...
	public void createTag(Context ctx, long projectId, String tagName, String ref, String message) {
		isActionLoading.setValue(true);
		RetrofitClient.getApiInterface(ctx)
				...
				.enqueue(
						new Callback<>() {
							@Override
							public void onResponse(
									@NonNull Call<TagsItem> c, @NonNull Response<TagsItem> r) {
								ApiResponseHandler.handleAction(
										r, isActionLoading, actionSuccess, error);
							...
							@Override
							public void onFailure(@NonNull Call<TagsItem> c, @NonNull Throwable t) {
								isActionLoading.setValue(false);
								error.setValue(t.getMessage());
							}
						});
